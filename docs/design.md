...
- [~] Re-measurement on a larger, more genre-diverse corpus: **cross-genre accuracy done** (Section 4.8). Added UD_Japanese-PUD (out-of-domain, different genre, 1000 sentences / 27,788 boundaries) and applied the GSD-trained models without retraining. No accuracy degradation for either KyTea or MLP (both score higher on PUD), and the MLP–KyTea gap is stable at 0.6–0.7pt (GSD-only 0.74pt → PUD 0.62pt → combined 0.67pt), confirming the initial 0.7pt as representative across 3× the data and 2 genres. Fetch via `scripts/fetch_ud_pud_corpus.sh`. Remaining: multi-genre *speed* re-measurement (the Section 9.4 in-process benchmark still uses only 710K characters of Aozora Bunko = literature; low priority since segmentation speed is genre-insensitive)
