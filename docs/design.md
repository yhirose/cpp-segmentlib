...
- The training engine itself (LIBLINEAR equivalent) is out of scope for this library; only **inference** is supported (Section 10 makes this final: neither self-implementing nor integrating externally will be pursued).
...
This library is segmentation-only (no tag estimation, decided in Section 10). `Segments` reduces to the minimal shape: a list of word-span pairs (start/end UTF-8 byte offsets) — no dedicated `Segment` struct, just a bare alias like `Boundaries`.
...
- A single `tokenize()` covering segmentation only. There used to be a "segmentation + tag estimation" `tokenize()` and a "segmentation-only fast path" `tokenize_boundaries()`, split at the type level; once tag estimation was removed the reason for that speed difference disappeared (boundary scoring is the same either way), and the two functions carry equivalent information (`Segments` yields boundary positions via `.second`), so they were merged (Section 10).
...
As with KyTea / Vaporetto, this follows the **filter-style pattern of reading text from standard input and writing segmentation results to standard output**. This library is segmentation-only (no tag estimation, decided in Section 10).

```
segmenter predict --model model.bin < input.txt > output.txt
```

**Options (implemented only)**
...
`--backend` (override auto-detection) and `--scores` (per-boundary score output) from the initial proposal remain unimplemented. `--boundaries-only` (segmentation-only fast path) and `--notags` (suppress tags) were **removed as no longer needed**, following the removal of tag estimation itself (Section 10) — `tokenize()` performs segmentation only from the start. `--encode` was decided not to be implemented (Section 10, UTF-8 fixed).

**Output format**

Follows KyTea's segmentation output format (a space-separated word list) to preserve compatibility with existing toolchains.
...
**The above is the initial proposal's option shape (a straightforward `--`-long-form mapping of `train-kytea`'s `-charw` etc.); Section 10 has decided against implementing it.** The current CLI (`train_command.cpp`) explicitly treats `--backend kytea` / `--backend vaporetto` as "not implemented" errors, and only `--backend mlp` is implemented. Where a KyTea-compatible model is needed (e.g. for this library's own evaluation/comparison use), the standard practice is to call the real `train-kytea` (Homebrew-distributed) directly as an external tool (Section 4.8's evaluation pipeline is a working example).
...
**L3: `kytea::Model`** — an immutable value type corresponding to `Config`/`KyteaModel`/`FeatureLookup` from Section 3.2. `static auto load(std::filesystem::path) -> std::expected<Model, Error>` is the sole construction path. It holds `charDict`/`typeDict`/`dictVector`/`biases`/`multiplier`/`word_dict` (only `char_length`/`in_dict`) — the minimum segmentation needs. Per-word tag information in the word dictionary, the subword dictionary (`ProbTagEntry`), the language model (`KyteaLM`), and the tag models (`globalMods_`) **are present in the file, so parsing must skip past them to reach the correct subsequent seek position** (since the Section 3.2 section order is fixed, an implementation that "just counts bytes and skips" is needed), but are not retained as values (tag estimation was implemented and then reverted; see Section 10's final decision).
...
`segmenter predict --model kytea-model.bin < input.txt > output.txt` was originally assumed to imply output including tag (reading) estimation, but tag estimation requires a full-blown implementation including the word dictionary, subword language model, and tag models, which is a large scope (the `tagDictVector`/`tagUnkVector` of `FeatureLookup` from Section 3.2). **As the first implementation step, it was reasonable to first get segmentation-only working with `Model` able to skip the tag-related sections**, and the MVP was built on that judgment.

**What happened next (Section 10)**: tag estimation was in fact implemented and verified byte-for-byte against real KyTea, but was ultimately **reverted, making this library segmentation-only**. So the above "MVP is segmentation-only first" judgment was not a temporary ordering choice — it became the final shape.
...
- [~] **Tag estimation, steps A-1/A-2/B**: implemented, verified byte-for-byte against real KyTea's default (tagged) output, and SIMD-optimized — but **later reverted** (Section 10) as part of the decision to make this library segmentation-only. `tag_scorer.{h,cpp}` and the model loader's tag-retaining code have been deleted; see the deletion commit and the (also deleted) `tag_prediction_plan.{ja.,}md` for the historical record
- [x] **L5: `kytea::KyteaBackend`** / **L6: `Segmenter`** (`std::variant` dispatch, `string_view` input + `std::expected`, Sections 2/7). Segmentation only — a single `tokenize()` (the old `tokenize_boundaries()` was merged in when tag estimation was reverted, Section 10)
- [x] **CLI: `segmenter predict`** (`--model`/`--threads`, stdin→stdout filter; `segmenter train` implements only `--backend mlp`, `--backend kytea` remains unimplemented, Section 7). Output is segmentation only (`append_full_line` = surface only, space-separated). The old `--notags`/`--boundaries-only`/`append_tagged_line` were removed when tag estimation was reverted (Section 10)
- [x] **Golden tests (byte-for-byte agreement with real KyTea output, Section 8.4)**: checked against `kytea -notags` using a fixed set of fixture sentences. **Skipped when the model has not been fetched** (CI-resilient)
...
### 9.2.1 (Reverted) Speed Including Tag Estimation

Tag estimation (steps A-2/B) was implemented, verified, and optimized, but was later reverted (the decision to make this library segmentation-only, Section 10). The tagged-speed measurements that used to live here (≈8x slower than WS-only, the word_dict load-cost breakdown, the round 12/13 optimization notes) were removed along with the code. See the deletion commit (`git log`) for details.
...
- **Tried but not adopted, round 11: flattening `Automaton<Payload>`'s payload** (a negative result). Right after completing tag-estimation steps A-2/B, seeing that tagged `tokenize` was 65–70x slower led to the hypothesis that "`addTagNgrams` (POS lev0 fires for every word) re-scans a small window per word, and `Payload=FeatVec` (`vector<vector<int16_t>>`)'s individual heap allocations cause scattered references, i.e., cache misses." An optimization was implemented that, when `Payload` is `std::vector<T>`, flattens all payloads into a contiguous buffer plus offset array and returns a `span`. **However, two problems came to light and it was withdrawn**:
  ...
  - **Lesson**: the moderate complexity of template duplication via `if constexpr`, branching return types (`span` vs. reference), and the latent pitfall of "empty span = not-found," was introduced before A/B measurement. **Should have measured correctly in `Release` first and confirmed the effect before adding complexity.** Withdrawn, reverting to the plain `vector<Payload>`.
- **(Reverted) Round 12: tag estimation's top-candidate argmax-ization** and **(Reverted) Round 13: SIMD-kernelizing tag estimation's widening-add loop**: both were implemented and measured, but were deleted along with `tag_scorer.cpp` when tag estimation itself was reverted (the decision to make this library segmentation-only, Section 10). See the deletion commit for details.
...
**Decision: this library is segmentation-only; tag estimation (POS/reading) is not performed.** The KyTea backend's tag estimation (steps A-1/A-2/B) had been implemented — verified byte-for-byte against real KyTea, SIMD-optimized — but was reverted for: (1) simplifying scope to match the library's name (segment = word segmentation), (2) simplifying `Segments` from `Segment{start,end,tags}` to `std::vector<std::pair<size_t,size_t>>` and merging `tokenize`/`tokenize_boundaries` into one function, (3) adding tag estimation to the MLP backend was judged too large (POS is medium-scale; reading is an open-vocabulary generation problem for unknown words with genuine research risk), leaving the two backends asymmetric in tag support. `tag_scorer.{h,cpp}` and `docs/tag_prediction_plan.{ja.,}md` have been deleted. The KyTea model file's tag-related sections (global tag models, per-word tag info, subword dictionary, language models) are still correctly skipped by byte count (Section 3.2) — the file format itself cannot change.
...
- [~] (Reverted) `--boundaries-only`/`--notags` wiring and the tag-skipping fast path: both flags were removed along with tag estimation itself (`tokenize()` performs segmentation only from the start, Section 10)
- [x] Multiple-candidate + confidence output (KyTea `-out conf`/`-tagmax`): moot — tag estimation itself was reverted (Section 10's final decision)
...
- [x] Training feature (whether to self-implement the KyTea-compatible training engine, or interface with external LIBLINEAR): **decided to do neither**. This makes final the scope Section 3 held from the start — the KyTea backend is inference-only. Reasons: (1) the custom MLP backend already has a complete, working, self-implemented training engine (Section 4.9, design 5.8 results) — training functionality already exists in this library. (2) Wherever a KyTea-compatible model has actually been needed (this library's own evaluation/comparison, generating `corpus/ud-gsd/kytea.mod`, etc.), the real `train-kytea` (Homebrew-distributed, which itself embeds LIBLINEAR) has always been called directly as an external tool, and that has been sufficient. (3) Unlike feature extraction — which Section 3.1 established must be reproduced byte-for-byte — the classifier itself is generic (Section 3's own framing), so there is no correctness reason to reimplement it; using the external reference implementation directly is the right way to avoid reinventing that wheel. `segmenter train --backend kytea` will not implement the Section 7.2 initial-proposal option shape, and stays the current explicit "not implemented" error (`train_command.cpp`)
...
- [~] (Reverted) Model-load breakdown including tag estimation, top-candidate argmax-ization (round 12), and `add_tag_ngrams` SIMD-kernelization (round 13): all were measured and implemented, then deleted along with tag estimation itself (Section 10's segmentation-only decision)
