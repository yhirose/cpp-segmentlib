...
- Speeding up tag estimation itself: argmax-ization of top-candidate selection has already been done (Section 9.4 round 12, +10–20%). Payload flattening was found to have no effect via A/B measurement and was withdrawn (round 11 = a negative result). `add_tag_ngrams`'s widening-add loop (about 31% in Release profiling) has been SIMD-kernelized (round 13, 1.65× on the loop itself; end-to-end awaits an idle re-measurement). The precomputation scheme was rejected on review — accumulation, not traversal, is the bottleneck (Section 10 TODO). Currently about 8x slower than WS-only, but still faster than KyTea's actual tagged measurement.
...
- **Round 13: explicit SIMD kernel for tag estimation's widening-add loop** (the `add_tag_ngrams` speedup from the Section 10 TODO — the largest cost in tag estimation, measured at ~18% of the total). The widening add `int32 scores[j] += int16 vec[pos+j]` (nw=21) is not auto-vectorized by the compiler because of the runtime bound (already established: `__restrict` did not change this). A widening-add kernel `add_widen_i16_i32` was added to the MLP side's `kernels.h` (which already provides the scalar-oracle + NEON/AVX2/scalar dispatch + bit-exactness-test infrastructure), and `tag_scorer.cpp` now uses it at 4 sites (`add_tag_ngrams`'s per-match accumulation, `add_self_weights`, `add_tag_dict_weights`, and the bias add). The kernel is a plain integer op, not MLP-specific, so it stays in kernels.h (the KyTea-backend → mlp/kernels.h cross-module dependency is noted in the header comment). **Byte agreement is guaranteed by construction** (integer adds are exact) — confirmed via golden, a byte-for-byte diff of old-vs-new binaries on 20,822 lines of Aozora Bunko, and all 115 tests green. **The loop itself measures 1.65× in an in-process interleaved A/B** (min-time, runtime nw=21, same access pattern as add_tag_ngrams): scalar 1.71 ms vs. NEON 1.04 ms, reproduced twice with a load-robust methodology. Expected end-to-end effect: 18% × (1 − 1/1.65) ≈ **7%**. **The end-to-end A/B is still unconfirmed**, however: the machine was heavily loaded at measurement time (load average 31 from builds/node processes; throughput swung 0.24–0.96 M/s, ±50%), so whole-`tokenize` confirmation awaits an idle re-measurement. The difference vs. the single-width experiment (+9–25%) is that the latter also removed 20 elements' worth of loads (the caveat already recorded in the Section 10 TODO).
...
- [~] Speeding up `add_tag_ngrams` (about 31% in Release profiling — the largest cost in tag estimation): **explicit SIMD kernel implemented** (Section 9.4 round 13). The 21-wide int16→int32 widening add (measured at ~18% of the total via the single-width A/B; not auto-vectorized even with `__restrict`) is replaced by `add_widen_i16_i32` added to `kernels.h` (scalar/NEON/AVX2, with a bit-exactness test). Byte agreement is guaranteed by the exactness of integer addition (confirmed via golden + a 20K-line Aozora diff + all 115 tests). The loop itself is **1.65×** in an in-process A/B (expected end-to-end ≈ 7%). **Only the end-to-end A/B remains unconfirmed** (the machine was heavily loaded at measurement time; re-measure when idle). The alternative that had been noted for consideration — a precompute scheme scanning the whole sentence once — is **now rejected on review**: the scan target is only the word's outer ±window (at most 6 chars with char_n=3), the real bottleneck is the per-match payload accumulation (which cannot be precomputed away since each word has a different match set), so the idea attacked a non-bottleneck term; junction-straddling matches (n-grams spanning the concatenated before/after-word context) also cannot be reproduced by per-position precomputation, making byte agreement risky
