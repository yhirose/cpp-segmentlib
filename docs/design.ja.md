...
- **EDLA比較用の素のSGD**：未実装。AdamはこのアーキテクチャにおいてEDLAの更新と誤差逆伝播の差の大部分を正規化で打ち消してしまうため、測定された同等性は見た目ほど強い証拠ではない。学習則単体の価値を確定するには、パラメータごとの再スケールを行わない最適化手法での比較が要る（11.6節）。
...
**最適化手法。** MLPバックエンドと同じAdamを、同じミニバッチ上で同じ`--lr`で使う。EDLA論文がより多く用いる素のSGDにすると最適化手法の変更が比較に混入するため。Adamのパラメータごとのステップはそのパラメータ自身の履歴のみから計算されるので、局所性という性質を壊さない。`Gradients`は入れ物として再利用している。ここでの値は損失の偏微分ではなく、Adamの減算が期待する符号を持たせた局所更新方向である。
...
**正直な読み方**：この深さ・この最適化手法では、信用割り当て規則はこのタスクで測定可能な差を生まない。EDLA単体の価値を確定するには、`|w2_j|`のスケールが正規化されない素のSGDでの同じ比較が必要である。これは未実装（10節）。
