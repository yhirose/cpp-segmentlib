...
- 学習エンジン自体（LIBLINEAR相当）は本ライブラリのスコープ外とし、まずは**推論のみ**をサポートする。
...
学習エンジンをどう実装するか（LIBLINEAR相当を自前実装するか外部連携するか）は9節のTODOのままだが、**コーパスの読み込み・素性抽出・CLIの引数形状は3節・5節の調査結果からすでに一意に決まる**ため、実装順としては「コーパスパーサ＋素性抽出（学習側でも推論側と共通）」→「線形分類器の学習（未着手部分）」という分割がしやすい。
...
- タグ推定自体の高速化：先頭候補選択のargmax化は実施済み（9.4節 第12弾、+10〜20%）。payload平坦化はA/B計測で効果なしと判明し撤回済み（第11弾＝負の結果）。残る本丸は`add_tag_ngrams`（Releaseプロファイルで約31%）だが、語ごとの窓再走査そのものを減らす事前計算方式は未検討。現状WSのみ比で約8倍低速だがKyTeaのタグ付き実測より速いため優先度は低い。
...
- [ ] より大規模・多ジャンルのコーパスでの再計測（現状71万字・文学中心）
...
- [ ] `add_tag_ngrams`の高速化（Releaseプロファイルで約31%＝タグ推定の最大コスト）。**内訳を実測で分解**：加算ループを1幅に削るA/Bで`tokenize`全体が一貫して+9〜25%速くなり（best 0.80→0.95 M/s）、**21幅の加算ループが全体の約18%＝加算律速**と判明（当初の「走査律速」という見立ては誤り。WSの6幅と違い、int16→int32の21幅拡張加算が効く）。ただし**エレガントな手は見つからず**：`__restrict`ポインタ化はA/Bで効果なし（コンパイラが拡張加算＋実行時境界nw=21を自動ベクトル化しない、best base 0.891≧vec 0.855）。18%を取るには明示的なNEON intrinsics（21要素の端数処理込み）が必要で非エレガント、かつ純粋計算SIMDで全18%取れる保証も未検証（1幅化実験は20要素ぶんのロード削減も含むため）。優先度低（現状WSのみ比約8倍低速だがKyTeaのタグ付き実測より速い）。別軸として、語ごと再走査そのものを減らす事前計算方式（文全体を1回走査し各語の窓をO(1)参照）も未検討（9.2.1節）
