...
**バージョン**：形式1は辞書を学習時の語リストとして保持し、ロードのたびにマッチャへコンパイルしていた。トレーナが現在書き出す形式2はそのマッチャ自体を保持する。57万語のUniDic辞書ではモデルが7.6MB→2.1MB、ロードが510ms→135msになる（後者は辞書なしのモデルと同じコスト）。差分はフィールド17だけで、ローダは形式1も引き続き読む。
...
| 17b | `fst` | `uint8 × fst_size` | cpp-fstlibのバイトコード。各エントリの正規化UTF-8バイト列をキーとし、出力はチャンネル集合のid。そのまま使う（ローダは展開も再構築もしない） |
...
形式1ではフィールド17はチャンネルごとに`entry_count`（`uint32`）とその個数のNUL終端UTF-8表層だった。

**ロード時の処理**：(1) 語彙・埋め込み・重みを読み、(2) 位置別事前計算テーブルと辞書素性の列ベクトルの展開を構築、(3) 形式1では単語リストを辞書FSTへコンパイル（形式2はファイル中のFSTをそのまま使う）、(4) `b1`/`b2`をアキュムレータ整数スケールへ量子化。
...
効くフィルタは1つ、**1文字エントリの除去**である。UniDicの1%にすぎないが実文中で絶えずマッチする一方、該当文字は文字窓が既に直接見ているため、除いても精度は測定可能な範囲で変わらず速度が約12%戻る。もう1つの候補だった語長4文字での打ち切りは形式2で役目を終えた。長いエントリの保存コストを生んでいた接頭辞・接尾辞をFSTが共有するため、打ち切りの節約はモデル半減ではなく0.4MBにとどまり、GSD −0.06pt / PUD −0.11ptを払う価値がない。同じ理由でロード時間も辞書サイズにほぼ依存しなくなった（上表のどの行も130〜137ms。形式1では最大505msだった）。
