...
std::expected<Boundaries, Error>
MlpBackend::tokenize_boundaries(std::string_view text) const {
    Scratch& s = scratch();
    ...
    for (std::size_t i = 0; i < s.scores.size(); ++i) {
        if (s.scores[i] > 0) {
            ...
            cuts.push_back(s.enc.offsets[i + 1]);
        }
    }
    return cuts;
}
...
std::expected<Segments, Error> MlpBackend::tokenize(std::string_view text) const {
    const auto cuts = tokenize_boundaries(text);
    if (!cuts) {
        return std::unexpected(cuts.error());
    }
    Segments segments;
    if (text.empty()) {
        return segments;
    }
    segments.reserve(cuts->size() + 1);
    std::size_t start = 0;
    for (const std::size_t cut : *cuts) {
        segments.push_back(Segment{start, cut, {}});
        start = cut;
    }
    segments.push_back(Segment{start, text.size(), {}});
    return segments;
}
